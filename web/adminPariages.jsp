...
<%
    ...
    if (request.getAttribute("paiementError") != null) {
%>
<p class="error" style="color: red"><%= request.getAttribute("paiementError")%></p>
...
<table border="3" cellspacing="3" cellpadding="3">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <% if (p.CheckGain()&& !p.getEtat().equals("P")) {
                %>
                <form method="post" action="<%= request.getContextPath()%>/admin/paiement">
                    <input type="hidden" name="montant" value="<%=p.getSolde_fiche()%>">
                    <input type="submit" name="button" value="Payer">          
                    <input type="hidden" name="id_pariage" value="<%=p.getCode_Pariage()%>">
                </form>
                <%} %>
            </td> 
        </tr>                
